...
static void select_file_path(const char *path, const char *label) {
    player.select_file(path, label);
}
    ...
    if (cmd.length() == 1) {
        switch (first_char) {
            ...
            case 'L':
                if (player.is_playing() || player.state() == PlayerState::PLAYING || player.state() == PlayerState::PAUSED) {
                    LOG_INFO("Armo nuovo file: stop corrente e ricarico selezione.");
                    player.stop();
                }
                player.arm_file();
                break;
            ...
            case 't':
            case 'T':
                select_file_path(kTestFilePath, " (test)");
                break;
            case 's':
            case 'S':
                select_file_path(kSampleFilePath, " (sample)");
                break;
            case 'i':
            case 'I':
                player.print_status();
        ...
        if (first_char == 'v') {
            ...
            if (new_path.length() == 0) {
                LOG_WARN("Percorso file non valido");
                return;
            }
            if (new_path.charAt(0) != '/') {
                new_path = "/" + new_path;
            }
            player.select_file(new_path.c_str());
            LOG_INFO("File selezionato: %s (richiede 'l' per caricare)", new_path.c_str());
        } else {
            LOG_WARN("Unknown command: %s. Type 'h' for help.", cmd.c_str());
        }
    }
...
void setup() {
    ...
    if (!LittleFS.begin()) {
        LOG_ERROR("Mount LittleFS fallito. Carica il filesystem con 'pio run -t uploadfs'.");
        return;
    }

    player.select_file(kTestFilePath, " (test)");
